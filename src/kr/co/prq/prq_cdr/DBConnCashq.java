...
public class DBConnCashq {
	...
	/**
	 * DB가 연결된 상태인지 체크하여 연결되지 않았으면 연결한다. 또한 연결이 끊어진 경우 다시 연결을 시도한다. 하지만 DB가 안올라온
	 * 경우 계속 연결을 시도하지만 연결이 안되므로 유의해야 한다.
	 * 
	 * @throws SQLException
	 */
	public static void dbConCheck() {
		try {
			if (dbCon == null) {
				...
				CON_DRIVER = EnvCashq.getInstance().CON_DRIVER;
				CON_DOMAIN = EnvCashq.getInstance().CON_DOMAIN;
				CON_PORT = EnvCashq.getInstance().CON_PORT;
				CON_DBNM = EnvCashq.getInstance().CON_DBNM;
				CON_USER = EnvCashq.getInstance().CON_USER;
				CON_PWD = EnvCashq.getInstance().CON_PWD;
			...
			DBConnCashq.latest_warning = "ErrPOS013";
			dbCon = null;
		} catch (ClassNotFoundException sqex) {
			// Utils.logput(sqex.getMessage());
			Utils.getLogger().warning(sqex.getMessage());
			DBConnCashq.latest_warning = "ErrPOS014";
			dbCon = null;
		} catch (Exception e) {
			Utils.getLogger().warning(e.getMessage());
			Utils.getLogger().warning(Utils.stack(e));
			DBConnCashq.latest_warning = "ErrPOS015";
			dbCon = null;
			throw new RuntimeException(DBConnCashq.latest_warning);
		}
	}

	/**
	 * jdbc:mysql://localhost:3306/sktl 와 같은식으로 리턴한다.
	 * 
	 * @return
	 */
	...
	public static void close() {
		try {
			...
			Utils.getLogger().warning(e.getMessage());
			DBConnCashq.latest_warning = "ErrPOS016";
		} catch (Exception e) {
			Utils.getLogger().warning(e.getMessage());
			Utils.getLogger().warning(Utils.stack(e));
			DBConnCashq.latest_warning = "ErrPOS017";
		}
		dbCon = null;
	}

	/**
	 * safen_cmd_hist_YYYYMM테이블의 존재성을 판단한 후 존재하지 않으면 테이블을 생성한다. 또한 존재하지 않으면 해당
	 * 월의 이름으로 된 로그파일을 지운다.
	 * 
	 * @return
	 */
	public static String isExistTableYYYYMM() {
		...
		String hist_table = "prq_cdr_" + Utils.getYYYYMM();
		...
		if (isExistLogTable == false) {
			...
			try {
				stmt.execute("create table " + hist_table
						+ " as select * from prq_cdr limit 0");// 테이블만 생성하고 데이터는
																// 옮기지 않는다.
			} catch (SQLException e) {
				Utils.getLogger().warning(e.getMessage());
				DBConnCashq.latest_warning = "ErrPOS018";
				e.printStackTrace();
			} catch (Exception e) {
				Utils.getLogger().warning(e.getMessage());
				Utils.getLogger().warning(Utils.stack(e));
				DBConnCashq.latest_warning = "ErrPOS019";
			}
		}
		return hist_table;
	}
	...
	private static boolean isExistTable(String tname) {
		...
		try {
			...
			e.printStackTrace();
			Utils.getLogger().warning(e.getMessage());
			DBConnCashq.latest_warning = "ErrPOS020";
		} catch (Exception e) {
			Utils.getLogger().warning(e.getMessage());
			Utils.getLogger().warning(Utils.stack(e));
			DBConnCashq.latest_warning = "ErrPOS021";
		} finally {
			dao.closePstmt();
		}
